...
class GAN(nn.Module):
    def __init__(self, gen_config, disc_config, pretrained_path=None):
        ...
        self.generator = self.__build_network(gen_config)
        self.discriminator = self.__build_network(disc_config)
    ...
    def __build_network(self, config):
        """
        Build a network based on the configuration.

        Args:
            config (list[dict]): Layer-wise network configuration.
        
        Returns:
            nn.Sequential: Sequential network model.
        """
        layers = []
        for layer in config:
            ...
            # Add Dropout if specified in the layer config
    ...
    def train_model(self, train_loader, epochs, warm_up, lr, weight_decay, device, early_stop=5, gen_update_freq=5, save_path=None):
        ...
        for epoch in range(epochs):
            ...
            combined_loss = gen_loss_epoch + disc_loss_epoch + abs(gen_loss_epoch - disc_loss_epoch)
            ...
            if gen_loss_epoch  < architecture_loss and epoch >= warm_up:
                ...
                stop_counter = 0
            elif combined_loss  >= architecture_loss:
                stop_counter += 1
